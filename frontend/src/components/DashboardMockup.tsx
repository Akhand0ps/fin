  ...
  return (
    <div className="w-full bg-white rounded-[2.5rem] border border-neutral-200 shadow-2xl overflow-hidden flex h-[500px] md:h-[600px] text-left relative">
          ...
          {[
            ...
            <div 
              ...
              className={cn(
                ...
                activeTab === item.label ? "bg-neutral-900 text-white shadow-lg" : "text-neutral-400 hover:text-neutral-900 hover:bg-neutral-100"
              )}
            >
      ...
      <div className="flex-1 flex flex-col bg-white overflow-hidden">
        {/* Header */}
        <header className="h-14 md:h-16 border-b border-neutral-50 flex items-center justify-between px-4 md:px-8 bg-white/50 backdrop-blur-sm sticky top-0 z-10">
        ...
        {/* Dashboard Content */}
        <div className="p-4 md:p-8 space-y-6 md:space-y-8 overflow-y-auto relative h-full">
          {activeTab === 'Overview' ? (
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              <div className="w-16 h-16 rounded-2xl bg-neutral-100 flex items-center justify-center text-neutral-400 mb-2">
                <ShieldCheck className="w-8 h-8" />
              </div>
              <h3 className="text-2xl font-bold text-neutral-900">Unlock {activeTab}</h3>
              <p className="text-sm text-neutral-500 leading-relaxed max-w-sm">
                This is a static sandbox. Sign up for a free account to get access to real-time analytics, investor management, and settings.
              </p>
              <button 
                onClick={() => window.location.href = '/register'}
                className="mt-4 bg-neutral-900 text-white px-6 py-2 rounded-xl text-sm font-bold shadow-lg hover:bg-neutral-800 transition-colors"
              >
                Create Free Account
              </button>
            </motion.div>
